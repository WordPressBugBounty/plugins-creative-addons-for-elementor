...
abstract class Creative_Widget_Base extends Widget_Base {
	...
	private function add_kb_selection() {
		...
		if ( empty($all_kb_configs) ) {
			...
			$this->start_controls_section(
				...
				[
					'label' => esc_html__( 'Knowledge Base', 'creative-addons-for-elementor' ),
					'tab' => Controls_Manager::TAB_CONTENT
				]
			);

				$text = esc_html__( "We can't find any Knowledge Base configuration. Ensure that your Echo Knowledge Base plugin is active.", 'creative-addons-for-elementor' );
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'crel_kb_id',
				[
					'label' => sprintf( esc_html__( 'Knowledge Base %s', 'creative-addons-for-elementor' ), '<a href="'.$wizard_url.'" target="_blank"><i class="crelfa crelfa-external-link "></i></a>' ),
		...
		if ( ! defined('EM'.'KB_PLUGIN_NAME') && count( $all_kb_configs ) == 1 ) {
			$this->add_notice_control( sprintf( esc_html__( 'Get %1$s Multiple Knowledge Bases %2$s add-on to create more Knowledge Bases.', 'creative-addons-for-elementor' ),'<a href="https://www.echoknowledgebase.com/wordpress-plugin/multiple-knowledge-bases/" target="_blank">', '</a>'), 'note' );
		}
	...
	public function sanitize_html_tag( $tag, $default='h2' ) {
		...
		$tag = trim( $tag );
		$tag = substr( $tag, 0, 2);
		return in_array( $tag, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'] ) ? $tag : ( in_array( $tag, ['1', '2', '3', '4', '5', '6'] ) ? 'h' . $tag : $default );
	}
	...
	private function add_standard_preset_options() {
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Presets', 'creative-addons-for-elementor' ),
				'tab' => Controls_Manager::TAB_CONTENT
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'buttons'       => true,
				'default_label' => esc_html__( 'Select Custom Preset', 'creative-addons-for-elementor' ),
				'first_empty'   => true
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' 		=> $presets,
				'reset'         => true,
				'default_label' => esc_html__( 'Select Preset', 'creative-addons-for-elementor' ),
				'first_empty'   => true,
		...
		$url = admin_url( 'admin.php?page=creative-addons&tab=presets&widget=' . $this->get_widget_class_name() );
		$this->add_control( 
			'crel_presets-link_' . $this->notice_id, [
				'label' => '',
				'type' => Controls_Manager::RAW_HTML,
				'raw' => '<div class="crel-presets-link"><a href="' . esc_url( $url ) . '" target="_blank">' . esc_html__( 'Preset Library', 'creative-addons-for-elementor' ) . '</a></div>',
			]
		);
	...
	/**
	 * Show this widget preset options
	 */
	private function add_extended_preset_options() {
		...
		$fake_select_options = '<option value="">' . esc_html__( 'Select ...', 'creative-addons-for-elementor') . '</option>';
			...
			[
				...
				<div class="elementor-control-field ' . ( $i ? '' : 'crel-with-reset' ) . '">
					...
					<div class="elementor-control-input-wrapper elementor-control-unit-5">' . ( $i ? '' :
						'<i title="' . esc_html__( 'Reset Style', 'creative-addons-for-elementor' ) . '" class="eicon-redo crel-reset-design" aria-hidden="true"></i>' ) .
		...
		$this->add_control( 
			'crel_presets-link_' . $this->notice_id, [
				'label' => '',
				'type' => Controls_Manager::RAW_HTML,
				'raw' => '<div class="crel-presets-link"><a href="' . esc_url( $url ) . '" target="_blank">' . esc_html__( 'Presets Library', 'creative-addons-for-elementor' ) . '</a></div>',
			]
		);
	...
	protected function kb_required_html() {
		...
		$message = KB_Utilities::is_kb_plugin_installed() ? sprintf( esc_html__( 'To use this widget, activate our free %s plugin.', 'creative-addons-for-elementor' ), '<b>Knowledge Base</b>' )
															: sprintf( esc_html__( 'To use this widget, please install our FREE %s plugin.', 'creative-addons-for-elementor' ), '<b>Knowledge Base</b>' ); ?>

		<div class="crel-kb_required">
			<div class="crel-kb_required-header"><?php echo esc_html( $this->get_title() ); ?> <?php esc_html_e( 'Widget', 'creative-addons-for-elementor' ); ?></div>
			<div class="crel-kb_required-body"><?php
				 echo wp_kses_post( $message ); ?></div>
			<div class="crel-kb_required-buttons"><?php
				if ( KB_Utilities::is_kb_plugin_installed() ) { ?>
					<a href="<?php echo esc_url( admin_url( 'plugin-install.php' ) . '?s=%22Echo%20Plugins%22&tab=search&type=term' ); ?>" class="crel-kb-button" target="_blank"><?php esc_html_e( 'Activate Plugin', 'creative-addons-for-elementor' ); ?></a><?php
				} else { ?>
					<a href="<?php echo esc_url( admin_url( 'plugin-install.php' ) . '?s=echoplugins&tab=search&type=author' ); ?>" class="crel-kb-button" target="_blank"><?php esc_html_e( 'Install Plugin', 'creative-addons-for-elementor' ); ?></a><?php
				} ?>

				<a href="<?php echo esc_url( $this->get_demo_url() ); ?>" class="crel-kb-button" target="_blank"><?php esc_html_e( 'Demo', 'creative-addons-for-elementor' ); ?></a>
			</div>
		</div>	<?php
	}

	protected function filter_image_url( $image_url ) {
		...
		// is image in the uploads folder
		$img_path = '';
		if ( strpos( $image_url, $base_url ) !== false ) {
			$rel_path = str_replace( $base_url, '', $image_url );
			$img_path = $base_dir . $rel_path;

		// is image in the current theme folder
		} else if ( strpos( $image_url, $theme_url ) !== false ) {
			$rel_path = str_replace( $theme_url, '', $image_url );
